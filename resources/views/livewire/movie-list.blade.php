    ...
    <div class="grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-3 gap-6">
        @forelse ($movies as $movie)
            <div class="p-4 bg-gray-800 rounded-lg shadow-2xl">
                {{-- <h3 class="text-lg text-slate-200 font-semibold">{{ Str::limit($movie->movieName,35) }}</h3> --}}
                ...
                <video controls class="w-full mt-2 rounded-md shadow-md">
                    ...
                    <button type="submit" wire:confirm="sure to delete this movie?" wire:click="deleteMovie({{$movie->id}})" class="mt-3 text-red-500 hover:text-red-700">Delete</button>
            </div>
        @empty
